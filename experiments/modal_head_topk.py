...
def run():
    ...
    ha, na, ta = pairs(H_a, T_a); hb, nb, tb = pairs(H_b, T_b)

    def train(Hx, Tx, secs):
        ...
        h, nxt, tgt = pairs(Hx, Tx); t0 = time.perf_counter()
        with torch.enable_grad():
            while time.perf_counter() - t0 < secs:
                ...
                for gp in opt.param_groups: gp["lr"] = 1e-3 * max(0.05, 1 - (time.perf_counter()-t0)/secs)
                opt.step()
        return a.detach(), b.detach()

    @torch.inference_mode()
    def topk(ab, hh, nn_, tt):
        a, b = ab[0].to(torch.bfloat16), ab[1].to(torch.bfloat16)
        c1=c2=c3=c5=n=0
        for i in range(0, hh.shape[0], 8192):
            x = torch.cat([hh[i:i+8192], E[nn_[i:i+8192]] * 30.0], -1)
            z = hh[i:i+8192] + F.linear(F.silu(F.linear(x, a)), b)
            top = F.linear(z, E).topk(5, -1).indices
            t = tt[i:i+8192][:, None]
            c1 += (top[:, :1]==t).any(1).sum().item(); c2 += (top[:, :2]==t).any(1).sum().item()
            c3 += (top[:, :3]==t).any(1).sum().item(); c5 += (top[:, :5]==t).any(1).sum().item(); n += t.shape[0]
        return c1/n, c2/n, c3/n, c5/n

    for k, secs in ((1, 12), (9, 12)):
        ...
        for nm, (hh, nn_, tt) in (("same-corpus", (ha, na, ta)), ("other-book", (hb, nb, tb))):
            r = topk(ab, hh, nn_, tt)
            print(f"RESULT [{k} prompt(s)] {nm}: top1 {r[0]:.3f}  top2 {r[1]:.3f}  top3 {r[2]:.3f}  top5 {r[3]:.3f}", flush=True)
