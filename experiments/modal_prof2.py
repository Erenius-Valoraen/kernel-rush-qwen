...
def prof(b: int, s: int, env: str):
    ...
    with torch.inference_mode():
        ...
        # decode step (eager launches; GPU kernel times are what the graph replays)
        from engine import _Runner
        eng.use_graphs = False
        r = _Runner(eng, st, 1, os.environ.get("PROF_PLAN", "fp8"))
        r.pos.fill_(s)
        for _ in range(3):
            r.step(eng, st)
        torch.cuda.synchronize()
        with profile(activities=[ProfilerActivity.CUDA]) as p:
            for _ in range(4):
                r.step(eng, st)
            torch.cuda.synchronize()
        print("DECODE x4 steps")
        print(p.key_averages().table(sort_by="cuda_time_total", row_limit=16, max_name_column_width=60))
