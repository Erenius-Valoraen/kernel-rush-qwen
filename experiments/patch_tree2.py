"""Sparse-tree speculation (ENGINE_TREE), corrected: layout [last, d1a, d2, d1b]
at contiguous slots p0..p0+3, logical RoPE rp=[0,1,2,1], prefix limits pl=[0,1,2,0],
self-slots ss=[0,1,2,3]. The a-branch (d1a->d2) is naturally contiguous; the
~13% b-branch moves d1b's one K/V vector from slot p0+3 to position p0+1."""
import ast
p = 'engine/engine.py'
s = open(p).read()
def rep(a, b):
...
rep('HEAD_MARGIN = 0.90', 'TREE = os.environ.get("ENGINE_TREE", "1") == "1"\nHEAD_MARGIN = 0.90')
